...
namespace CSUtils.Samples
{
  ...
  public class SampleScaler : Sample
  {
    ...
    SampleScaler(ValueArray MeasureTime, ValueMatrix Photons, SampleScalerParams sp)
        : base(sp)
    {
      if(Photons.Length != MeasureTime.Length)
        throw new ArgumentException("Arrays should correspond to each other");

      data = new DataColumn[4];
      ValueMatrix vm = (ValueMatrix)Photons.Clone();
      //     this.Nanometers = (ValueArray)Nanometers.Clone();
      this.MeasureTime = (ValueArray)MeasureTime.Clone();
      this.Photons = vm;
      this.Nanometers = vm.YAxis;
      this.KineticAxis = vm.XAxis;

      SP.Points = Length;
    }
    public SampleScaler(ValueArray Seconds, ValueMatrix Photons, params SettingsBase[] sets)
        : this(Seconds, Photons, new SampleScalerParams(sets)) { }
    public SampleScaler(string filename) : base(filename) { }
    public SampleScaler(Range Range, params SettingsBase[] sets)
  ...
  public class ValueMatrix : DataColumn
  {
    ...
    public ValueMatrix Clone(int? capacity)
    {
      ...
      nva.Length = Length;
      for(int i = 0; i < Length; i++) {
        nva.rows[i] = new double[RowCapacity];
        Array.Copy(rows[i], nva.rows[i], RowCapacity);
      }
      return nva;
    }
